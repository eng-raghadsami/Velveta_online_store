...
@section('title','Edit Ring | '. env('APP_NAME'))
@section('content')

<div class="d-flex justify-content-between">
    <h1 class="h3 mb-4 text-dark-800 ml-4">Edit Ring</h1>
    <a href="{{ route('dashboard.rings.index') }}" class="btn mb-4 mr-3" style="background-color: #834703;color: white;">All Rings</a>
</div>

<div class="card">
    <div class="card-body">
        <form action="{{ route('dashboard.rings.update', $ring->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            @if($ring->image)
                <div class="mb-3">
                    <img src="{{ asset($ring->image)}}" alt="Current Image" width="150">
                </div>
            @endif
            ...
            <x-input
                ...
                type="number"
                step="0.01"
                :value="old('sale_price', $ring->sale_price)"
            />
